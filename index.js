...
function updateUrlWithCanvasData(reload) {
  try {
    ...
    const newUrl = `${globalThis.location.pathname}?data=${encodeURIComponent(dataURL)}`;
    if (reload) {
...
function createNewCanvas(width_, height_, keepURL) {
  ...
  if (Array.from(resolutionSelect.options).some((option) => option.value === resolutionValue)) {
...
resolutionSelect.addEventListener("change", async function () {
  ...
  const id = await createImageBitmap(ctx.getImageData(0, 0, canvas.width, canvas.height));
...
fileInput.addEventListener("change", function (event) {
  ...
  reader.onload = function (e) {
    ...
    img.onload = () => {
      if (
        globalThis.confirm("選択した画像をキャンバスに読み込みますか？現在の内容は失われます。")
      ) {
...
try {
  if (dataParam) {
    if (dataParam.startsWith("data:image")) {
      ...
      img.onerror = () => {
        console.error("URLデータのデコードに失敗しました。無効な画像データです。");
